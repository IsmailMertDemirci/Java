package day21_30.day21_Arrays;

import java.util.Scanner;

public class C03_ArraydeContainsOlusturma {//normalde Stringe ait Contains methodunu arrayde olusturduk
    public static void main(String[] args) {
        //Kullanicidan aldiginiz bir ismin verilen arrayde olup olmadigini kontrol edip,
        // bize true veya false sonucu donen bir method olusturun.

        String[] isimler = {"Basak", "Nurullah", "Fatih", "Adem", "Enes"};
        Scanner scan = new Scanner(System.in);
        System.out.println("Aradiginiz ismi yaziniz");
        String arananIsim = scan.nextLine();

        boolean sonuc = contains(isimler, arananIsim);//isimlerde arayacak, ondan dolayi 2 tane parametre
        if (sonuc) {
            System.out.println("Girilen isim listede var");
        } else {
            System.out.println("Girilen isim listede yok");
        }
    }

    public static boolean contains(String[] isimler, String arananIsim) {

        boolean sonucMethod = false;

        for (int i = 0; i < isimler.length; i++) {
            if (isimler[i].equalsIgnoreCase(arananIsim)) {//herhangi bir eleman aranan isme esitse true yoksa zaten false
                sonucMethod = true;
            }
        }
        return sonucMethod;
    }
}
